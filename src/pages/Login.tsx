...
export default function Login() {
  ...
  /* ── Render ───────────────────────────────────────────────────── */
  return (
    <div className="bg-gradient-to-br from-[#FFFDF5] to-[#FFFDF5] min-h-screen flex items-center justify-center p-4">
      <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-xl border border-[#F0E6C8] w-full max-w-md">

        {/* Brand */}
        <div className="flex flex-col items-center mb-6">
          <div className="w-12 h-12 bg-yellow/30 rounded-2xl flex items-center justify-center mb-3">
            <Leaf size={24} className="text-yellow-dark" />
          </div>
          ...
          {redirectPath !== '/' && (
            <p className="mt-2.5 text-[11px] font-bold text-yellow-dark bg-yellow/10 border border-yellow-dark px-3 py-1 rounded-full">
              Sign in to continue
            </p>
          )}
        </div>
        ...
        {emailStep === 'input' && (
          <form onSubmit={handleSendLink} noValidate className="space-y-4">
            ...
            <button type="submit" disabled={loading}
              className="w-full bg-yellow-dark hover:bg-yellow-dark text-white font-bold py-3.5 rounded-xl transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
              {loading
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-white border-2 border-[#F0E6C8] hover:border-yellow-darkDark text-textMain font-bold py-3.5 rounded-xl transition-all disabled:opacity-60 shadow-sm hover:shadow-md active:scale-[0.98]"
            >
        ...
        {emailStep === 'sent' && (
          <div className="text-center space-y-5">
            <div className="w-16 h-16 bg-yellow/10 rounded-full flex items-center justify-center mx-auto">
            ...
            <div>
              ...
              <p className="text-[13px] text-textMuted leading-relaxed">
                ...
                <strong className="text-yellow-dark">{email}</strong>
              </p>
            </div>
        ...
        {/* Footer */}
        <div className="mt-6 pt-5 border-t border-[#F0E6C8]/50 text-center">
...
const inputCls = (hasError: boolean) =>
  `w-full px-4 py-3 rounded-xl border-2 outline-none text-[13px] transition-colors ${
    hasError
      ...
      : 'border-[#F0E6C8] focus:border-yellow-dark'
  }`
