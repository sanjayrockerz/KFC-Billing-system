...
const UNIT_TYPE_OPTIONS: { value: UnitType; label: string; hint: string }[] = [
  ...
  { value: 'bundle', label: 'Bundle / Set',    hint: 'e.g. meal combo, party pack' },
]
...
export default function Dashboard() {
  ...
  const loadCategories = useCallback(async () => {
    ...
    if (error) {
      setCategoryNotice({ type: 'error', text: error.message || 'Could not load categories.' })
      return
    }
    const byName = new Map<string, Category>()
    for (const category of ((data || []) as Category[])) {
      const key = category.name_en.trim().toLowerCase()
      if (!key || category.is_active === false) continue
      if (!byName.has(key)) byName.set(key, category)
    }
    setCats(Array.from(byName.values()))
  }, [])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const orderResult = await supabase.from('orders')
        .select('id, invoice_no, customer_name, phone, address, created_at, total, status, order_mode, order_type, user_id, items, coupon_code, discount_amount, manual_discount_amount, delivery_charge, total_gst, payment_mode, payment_method, invoice_pdf_url')
        .order('created_at', { ascending: false })
        .limit(1000)
      if (orderResult.error) {
        console.error('Dashboard orders load error', orderResult.error)
      } else {
        orderRows = orderResult.data || []
      }
      const mappedOrders = orderRows.map(r => toDashboardOrder(r as Record<string, unknown>))
  ...
  const generateCouponCode = () => {
    const prefixes = ['KFC', 'CRISPY', 'CHICKEN', 'FRY', 'COMBO', 'SPECIAL', 'FRESH']
  ...
  const saveCoupon = async (e: FormEvent) => {
    ...
    if (error) {
      ...
      if (msg.toLowerCase().includes('row-level security')) {
        setCouponSaveError('Coupon save blocked by Supabase RLS. Run supabase/migrations/20260715_0001_kfc_catalog_backend_sync.sql in Supabase SQL Editor.')
      } else {
  ...
  }, [tab, loadUsers, loadCoupons, loadCategories])
  ...
  const handleSaveProd = async (e: FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        image_url: prodForm.image || '/logo.png',
        image:     prodForm.image || '/logo.png',
      }
  ...
  const onAddCat = async (e: FormEvent) => {
    ...
    const payload = { ...newCat, name_en: newCat.name_en.trim(), name_ta: newCat.name_ta.trim() }
    let error: unknown = null
    if (editingCategoryId === null) {
      const existing = cats.find(category => category.name_en.trim().toLowerCase() === payload.name_en.toLowerCase())
      const result = existing
        ? await supabase.from('categories').update({ ...payload, is_active: true }).eq('id', existing.id)
        : await supabase.from('categories').insert({ ...payload, is_active: true, sort_order: cats.length + 1 })
      error = result.error
    } else {
      const result = await supabase.from('categories').update(payload).eq('id', editingCategoryId)
      error = result.error
    }
    if (error) {
      setCategoryNotice({ type: 'error', text: (error as { message?: string }).message || 'Could not add category.' })
      return
    }
  ...
  const toggleCat = async (c: Category) => {
    await supabase.from('categories').update({ is_active: !c.is_active }).eq('id', c.id); await loadCategories(); await fetchProducts(true)
  }
  ...
  const moveCat = async (c: Category, dir: 'up' | 'down') => {
    ...
    await supabase.from('categories').update({ sort_order: next }).eq('id', c.id); await loadCategories()
  }
  ...
  return (
    <div className="admin-shell h-full bg-bgMain flex flex-col lg:flex-row">
      ...
      <aside
        className={[
          'w-full bg-[#1A1A1A] text-white border-b lg:border-b-0 lg:border-r border-[#D4A800]/45 flex flex-col shrink-0 lg:min-h-screen',
      ...
      >
        ...
        <div className={`hidden lg:flex items-center relative transition-all duration-300 ${sidebarCollapsed ? 'justify-center pt-6 pb-5 px-2' : 'px-5 py-5'}`}>
          <div className={`flex items-center gap-3 min-w-0 transition-all duration-300 ${sidebarCollapsed ? 'w-0 opacity-0 overflow-hidden' : 'opacity-100 flex-1'}`}>
            <div className="flex items-center justify-center shrink-0 w-12 h-12 rounded-xl bg-white shadow-[0_4px_12px_rgba(17,24,39,0.10)] overflow-hidden">
              ...
              <h1 className="text-[20px] font-black text-[#F5C518] truncate tracking-tight">Korean Fried Chicken</h1>
          </div>
          <button
            type="button"
        ...
        <div className="flex lg:hidden items-center justify-between px-4 py-4 border-b border-white/10">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-[15px] font-black text-[#F5C518] truncate">Korean Fried Chicken</span>
          </div>
        </div>
        {/* Nav */}
        <nav
        ...
        >
          {navItems.map(item => (
            <button
              ...
              className={[
                ...
                tab === item.id ? 'bg-[#F5C518] text-[#111111] shadow-sm' : 'text-white/75 hover:bg-[#F5C518]/15 hover:text-[#F5C518]',
              ].join(' ')}
            >
          ...
          <button
            ...
            className={[
              ...
              sidebarCollapsed ? 'lg:w-[48px] lg:justify-center mx-auto' : 'lg:px-4',
              'px-0 py-1 lg:py-0',
              'rounded-xl font-medium text-[11px] lg:text-[14px] transition-all text-white/75 hover:bg-[#F5C518]/15 hover:text-[#F5C518] lg:mt-auto mb-1 lg:mb-4 overflow-hidden',
            ].join(' ')}
          >
      ...
      <main className="flex-grow flex flex-col overflow-hidden">
                    ...
                    <tbody className="divide-y divide-blue-50">
                      {analytics.onlineRequestOrders.map(order => {
                        ...
                        const waMsg = buildProfessionalWhatsAppMessage({
                          ...
                          phone: order.phone || '-',
                          invoiceNumber: order.invoice_no || order.id || '-',
                          invoiceDate: new Date(order.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }),
                          paymentMode: order.payment_mode || 'POS',
                          items: its.map(raw => {
                            const it = raw as Record<string, unknown>
                            const qty = toNumber(it.quantity ?? it.qty, 0)
                            const lineTotal = toNumber(it.line_total ?? it.lineTotal, 0)
                            return {
                              name: String(it.name || it.product_name || 'Product'),
                              qty,
                              unit: String(it.unit || 'pc'),
                              unitType: 'unit' as const,
                              rate: qty > 0 ? lineTotal / qty : lineTotal,
                              lineTotal,
                            }
                          }),
                          subtotal: toNumber(order.total, 0),
                    ...
                    <div className="h-48">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={analytics.monthlyTrend}>
                          ...
                          <Bar dataKey="revenue" fill="#F5C518" radius={[4, 4, 0, 0]} barSize={12} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-2xl border border-[#EAD7B7]/30 p-5 shadow-sm">
                  ...
                  <div className="mb-3">
                    <input
                      ...
                      className="w-full px-4 py-2.5 bg-[#F7F6F2] border border-[#EAD7B7]/60 rounded-xl text-[13px] font-bold text-[#2C392A] placeholder:text-[#8A9384] focus:outline-none focus:border-[#D4A800] transition-colors"
                    />
                  </div>
                  {(() => {
        ...
        {tab === 'history' && (
          <div className="space-y-4 sm:space-y-6 rounded-[20px] sm:rounded-[28px] border border-[#EAD7B7]/60 bg-[#FBFAF6] p-3 sm:p-6 lg:p-7 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
            ...
            <div className="rounded-2xl border border-[#EAD7B7]/60 bg-white p-3 sm:p-6 shadow-sm">
              {/* Bill type filter */}
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  {(['today', 'week', 'month', 'custom'] as const).map(preset => (
                    <button key={preset} type="button" onClick={() => applyDatePreset(preset)}
                      className={`min-h-[44px] px-3 py-1.5 rounded-xl text-[12px] font-black transition-colors ${datePreset === preset ? 'bg-[#D4A800] text-white shadow-sm' : 'bg-[#F7F6F2] text-[#5F6D59] hover:bg-[#EAD7B7]/40'}`}>
                  ...
                  {(search.dateFrom || search.dateTo || datePreset) && (
                    <button type="button" onClick={() => { setDatePreset(''); setSearch(s => ({ ...s, dateFrom: '', dateTo: '' })) }}
                      className="min-h-[44px] px-3 py-1.5 rounded-xl text-[12px] font-black text-[#D4A800] hover:bg-[#D4A800]/5">{l('Clear Dates', 'தேதி அழி')}</button>
                  )}
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  <input className="min-h-[48px] rounded-xl bg-[#F7F6F2] px-3 py-2.5 text-[16px] md:text-[13px] font-semibold text-[#2C392A] placeholder:text-[#8A9384] focus:outline-none focus:ring-2 focus:ring-[#D4A800]/15" placeholder={l('Invoice / Bill No', 'பில் எண்')}
                    value={search.invoiceNo} onChange={e => setSearch(s => ({ ...s, invoiceNo: e.target.value }))} />
                  <input className="min-h-[48px] rounded-xl bg-[#F7F6F2] px-3 py-2.5 text-[16px] md:text-[13px] font-semibold text-[#2C392A] placeholder:text-[#8A9384] focus:outline-none focus:ring-2 focus:ring-[#D4A800]/15" placeholder={l('Customer Name', 'வாடிக்கையாளர் பெயர்')}
                    value={search.customerName} onChange={e => setSearch(s => ({ ...s, customerName: e.target.value }))} />
                  <input className="min-h-[48px] rounded-xl bg-[#F7F6F2] px-3 py-2.5 text-[16px] md:text-[13px] font-semibold text-[#2C392A] placeholder:text-[#8A9384] focus:outline-none focus:ring-2 focus:ring-[#D4A800]/15" placeholder={l('Mobile Number', 'மொபைல் எண்')}
                    value={search.phone} onChange={e => setSearch(s => ({ ...s, phone: e.target.value }))} />
                  {datePreset === 'custom' ? (
                    <>
                      <input type="date" className="min-h-[48px] rounded-xl bg-[#F7F6F2] px-3 py-2.5 text-[16px] md:text-[13px] font-semibold text-[#2C392A] focus:outline-none focus:ring-2 focus:ring-[#D4A800]/15"
                    ...
                    <button type="submit" disabled={searchLoading}
                      className="sm:col-span-2 min-h-[48px] flex items-center justify-center gap-2 rounded-xl bg-[#D4A800] py-2.5 text-[13px] font-bold text-white shadow-sm transition-colors hover:bg-[#1A1A1A] disabled:opacity-60">
                      <Search size={14} /> {searchLoading ? l('Searching...','தேடுகிறது...') : l('Search Bills','தேடு')}
                    </button>
                  )}
                  {datePreset === 'custom' && (
                    <button type="submit" disabled={searchLoading}
                      className="sm:col-span-2 lg:col-span-4 min-h-[48px] flex items-center justify-center gap-2 rounded-xl bg-[#D4A800] py-2.5 text-[13px] font-bold text-white shadow-sm transition-colors hover:bg-[#1A1A1A] disabled:opacity-60">
                      <Search size={14} /> {searchLoading ? l('Searching...','தேடுகிறது...') : l('Search Bills','தேடு')}
                    </button>
                  )}
                </div>
              ...
              <div className="space-y-3 md:hidden">
                {filteredSearchResults.slice(0, 50).map(o => {
                  ...
                  return (
                    <div key={o.id} className="rounded-2xl border border-[#EAD7B7]/60 bg-[#FBFAF6] p-3 sm:p-4 space-y-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 pt-1">
                        ...
                        <button onClick={() => void deleteOrder(o.id, o.invoice_no)} className="h-10 w-10 sm:h-11 sm:w-11 shrink-0 rounded-xl border border-[#EAD7B7]/60 text-[#D4A800] transition-colors hover:bg-[#D4A800]/5" title="Delete Order">
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-[#EAD7B7] bg-[#FBFAF6] px-3 py-2 text-[11px] font-black text-[#D4A800] shadow-sm transition-colors hover:bg-[#F7F1E7]"
                >
                  <RefreshCw size={12} />
                  Refresh
                </button>
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                <div className="rounded-xl border border-[#EAD7B7] bg-[#FBFAF6] px-3 py-3 shadow-sm">
                  <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#D4A800]">Total Coupons</p>
                  <p className="mt-1 text-[20px] font-black text-[#2C392A]">{coupons.length}</p>
                </div>
                <div className="rounded-xl border border-[#EAD7B7] bg-[#FBFAF6] px-3 py-3 shadow-sm">
                  <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#D4A800]">Active</p>
                  <p className="mt-1 text-[20px] font-black text-[#D4A800]">{coupons.filter(c => c.is_active).length}</p>
                </div>
                <div className="rounded-xl border border-[#EAD7B7] bg-[#FBFAF6] px-3 py-3 shadow-sm">
                  <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#D4A800]">Used</p>
                  <p className="mt-1 text-[20px] font-black text-[#2C392A]">{coupons.reduce((acc, c) => acc + (c.usage_count || 0), 0)}</p>
                </div>
              </div>

              <div className="rounded-xl border border-[#E7CFAA] bg-[#FFF6E7] px-3 py-2 text-[11px] font-bold text-[#D4A800] shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
              <form onSubmit={saveCoupon} className="rounded-2xl border border-[#EAD7B7] bg-[#FFFCF6] p-4 shadow-sm space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#D4A800]">{editingCouponId !== null ? 'Edit mode' : 'New coupon'}</p>
                  ...
                  {editingCouponId !== null && (
                    <button
                      ...
                      className="rounded-full border border-[#E7CFAA] bg-[#FFF6E7] px-2.5 py-1 text-[11px] font-black text-[#D4A800] transition-colors hover:bg-[#FBEBD3]"
                    >
                      Cancel
                    </button>
                  )}
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    <input
                      className="flex-1 rounded-xl border border-[#E7D9BF] bg-white px-3 py-2.5 text-[12px] font-black uppercase tracking-[0.12em] text-[#2C392A] outline-none transition-colors focus:border-[#D4A800]"
                    ...
                    {editingCouponId === null && (
                      <button
                        ...
                        className="shrink-0 rounded-xl border border-[#D4A800] bg-[#D4A800] px-3 py-2.5 text-[11px] font-black text-white transition-colors hover:bg-[#1A1A1A]"
                      >
                        Generate
                      </button>
                    )}
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      min="1"
                      max="100"
                      className="w-full rounded-xl border border-[#E7D9BF] bg-white px-3 py-2.5 text-[12px] font-bold text-[#2C392A] outline-none transition-colors focus:border-[#D4A800]"
                      placeholder="10"
                ...
                <button
                  type="submit"
                  className="w-full rounded-xl bg-[#D4A800] py-3 text-[13px] font-black text-white shadow-sm transition-colors hover:bg-[#1A1A1A]"
                >
              ...
              <div className="rounded-2xl border border-[#EAD7B7] bg-[#FFFCF6] p-4 shadow-sm">
                <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="rounded-full border border-[#E7CFAA] bg-[#FFF6E7] px-2.5 py-0.5 text-[10px] font-black uppercase tracking-[0.15em] text-[#D4A800]">
                        ...
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                          <div className="min-w-0 space-y-1.5">
                            <div className="flex flex-wrap items-center gap-1.5">
                              ...
                              <span className={`rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-[0.14em] ${coupon.is_active ? 'bg-[#FCE7EA] text-[#D4A800]' : 'bg-[#F8EDD9] text-[#9A6700]'}`}>
                          ...
                          <div className="flex shrink-0 items-center gap-1.5">
                            ...
                            <button
                              ...
                              className="rounded-full border border-[#E7D9BF] bg-white p-2 text-[#D4A800] transition-colors hover:border-[#D8BA8A] hover:text-[#1A1A1A]"
                            >
                              <Edit2 size={14} />
                            </button>
                            <button
