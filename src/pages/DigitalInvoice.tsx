  ...
  useEffect(() => {
    async function loadInvoice() {
      ...
      try {
        ...
        // Try RPC lookup by invoice number first. If the public RPC has not
        // been applied yet, fall back to direct invoice_no/id lookup.
        ...
        if (!row || rpcError) {
          ...
          const directQuery = supabase
            .from('orders')
            .select('*')
          const { data: directData, error: directError } = await (isUuid
            ? directQuery.eq('id', identifier)
            : directQuery.eq('invoice_no', identifier)
          )
            .single()
          
          if (directError || !directData) throw new Error('Invoice not found')
          row = directData
        }
...
const invoiceItems = (Array.isArray(invoice.items) ? invoice.items : [])
  ...
  const shareViaWhatsApp = async () => {
    ...
    const publicInvoiceLink = `${window.location.origin}/invoice/${encodeURIComponent(invoice.invoice_no)}`
    const whatsappMessage = [
      message,
      '',
      `View / Download Invoice: ${publicInvoiceLink}`,
      downloadLink ? `PDF Backup: ${downloadLink}` : '',
    ].filter(Boolean).join('\n')
