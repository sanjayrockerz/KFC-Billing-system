...
export default function ProductDetails() {
  ...
  if (loading) return <div className="min-h-screen bg-[#fbfaf6] p-10 text-center font-bold text-[#1A1A1A]">Loading...</div>
    ...
    <div className="min-h-screen bg-[#fbfaf6] pb-[calc(7.75rem+env(safe-area-inset-bottom))]">
      <div className="lg:hidden sticky top-0 z-30 border-b border-white/60 bg-[#fbfaf6]/92 px-4 py-3 backdrop-blur sm:px-6">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3">
          <Link
            to="/products"
            className="inline-flex items-center gap-2 rounded-full border border-[#F0E6C8]/60 bg-white px-3 py-2 text-[11px] font-black text-[#1A1A1A] shadow-sm"
          >
      ...
      <div className="lg:hidden mx-auto flex max-w-3xl flex-col gap-0 px-0 sm:px-6 lg:px-8">
        ...
        <section className="px-4 pt-4 sm:px-0 sm:pt-5">
          <div className="space-y-2">
            ...
            <h1 className="text-[1.55rem] leading-tight font-black text-[#1A1A1A] sm:text-4xl">{displayName}</h1>
            {product.nameTa && <p className="text-base font-bold text-[#6B7280] ta-text sm:text-lg">{product.nameTa}</p>}

            <div className="flex flex-wrap items-center gap-2 pt-1">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-[11px] font-black text-[#1A1A1A] shadow-sm ring-1 ring-[#F0E6C8]/50">
                <Star size={12} className="fill-amber-400 text-amber-400" />
                {(product.rating || 4.7).toFixed(1)}
              </span>
              <span className="inline-flex items-center gap-2 rounded-full bg-[#f7f4ed] px-3 py-1.5 text-[11px] font-black text-[#6B7280] shadow-sm ring-1 ring-[#F0E6C8]/45">
                <span className="text-[#D4A800]">{formatCurrency(basePrice)}</span>
        ...
        <section className="px-4 pt-4 sm:px-0">
          <div className="rounded-[24px] bg-white/95 p-3.5 shadow-sm ring-1 ring-[#F0E6C8]/55 sm:p-4">
            <AnimatePresence mode="wait">
              {mobileQty === 0 ? (
                <motion.button
                  ...
                  type="button"
                  className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#1A1A1A] py-3 text-sm font-black text-white shadow-[0_16px_30px_rgba(44,57,42,0.22)]"
                >
                ...
                >
                  ...
                  <div className="inline-flex w-full items-center justify-between gap-2 rounded-full bg-white px-2 py-1 shadow-sm ring-1 ring-[#F0E6C8]/55">
                    <button
                      type="button"
                      onClick={() => handleMobileChangeQty(mobileQty - 1)}
                      className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f7f4ed] text-[#6B7280]"
                    >
                      <Minus size={13} />
                    </button>
      ...
      <div className="hidden lg:block mx-auto flex max-w-3xl flex-col gap-0 px-0 sm:px-6 lg:px-8">
        ...
        <section className="px-4 pt-4 sm:px-0 sm:pt-5">
          <div className="space-y-2">
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#D4A800]">{t('cat.' + product.category)}</p>
            <h1 className="text-[1.8rem] leading-tight font-black text-[#1A1A1A] sm:text-4xl">{displayName}</h1>
            {product.nameTa && <p className="text-base font-bold text-[#6B7280] ta-text sm:text-lg">{product.nameTa}</p>}
          ...
          <div className="rounded-[24px] bg-white/95 p-3.5 shadow-sm ring-1 ring-[#F0E6C8]/55 sm:p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="text-right">
                <p className="text-[10px] font-bold text-[#D4A800]">Price</p>
                <p className="text-lg font-black text-[#1A1A1A]">{formatCurrency(lineTotal)}</p>
              </div>
            </div>
        ...
        <section className="px-4 pt-4 sm:px-0">
          <div className="grid gap-2.5">
            <details className={accordionClass}>
              ...
              <p className="mt-3 text-sm leading-relaxed text-[#6B7280]">{displayDesc || 'Carefully selected herbal product made for daily use.'}</p>
            </details>
        ...
        <section className="px-4 pt-4 sm:px-0">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-[11px] font-black uppercase tracking-[0.24em] text-[#D4A800]">Related products</h3>
          ...
          <div className="mt-3 flex gap-3 overflow-x-auto pb-2 hide-scrollbar">
            ...
            {relatedProducts.map((item) => (
              <button
                ...
                className="group min-w-[124px] overflow-hidden rounded-[20px] bg-white text-left shadow-sm ring-1 ring-[#F0E6C8]/55 transition-transform hover:-translate-y-0.5"
              >
                ...
                <div className="space-y-1.5 p-2.5">
                  <p className="line-clamp-2 text-[11px] font-bold leading-snug text-[#1A1A1A]">{item.name}</p>
                  <p className="text-[10px] font-black text-[#D4A800]">{formatCurrency(item.offerPrice || item.price)}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-[#F0E6C8]/50 bg-white/95 px-4 py-3 backdrop-blur pb-[calc(env(safe-area-inset-bottom)+0.75rem)]">
        <div className="mx-auto flex max-w-3xl items-center gap-3">
          <div className="min-w-0">
            <p className="text-[11px] font-bold text-[#D4A800]">Total</p>
            <p className="text-base font-black leading-tight text-[#1A1A1A]">{formatCurrency(lineTotal)}</p>
          </div>
          <button
            onClick={handleAdd}
            className="flex-1 rounded-2xl bg-[#1A1A1A] py-3.5 text-sm font-black text-white shadow-[0_16px_30px_rgba(44,57,42,0.28)]"
            type="button"
          >
