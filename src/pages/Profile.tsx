        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1">
            <div className="surface-panel p-6">
              {/* Avatar */}
              <div className="flex flex-col items-center text-center mb-6 pb-6 border-b border-[#F0E6C8]">
                <div className="relative mb-3">
                  <div className="w-20 h-20 rounded-full overflow-hidden bg-gradient-to-br from-yellow to-yellow-dark flex items-center justify-center">
                    {avatarUrl ? (
                  ...
                  {isSupabaseConfigured && (
                    <button
                      ...
                      className="absolute bottom-0 right-0 w-7 h-7 bg-yellow-dark hover:bg-yellow-dark text-white rounded-full flex items-center justify-center shadow-md transition-colors disabled:opacity-60"
                ...
                <span className={`mt-2 px-3 py-0.5 rounded-full text-xs font-bold ${user.role === 'admin' ? 'bg-yellow-dark/20 text-yellow-dark' : 'bg-blue-100 text-blue-700'}`}>
              ...
              <div className="space-y-2">
                <button onClick={() => setActiveTab('orders')}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-colors ${activeTab === 'orders' ? 'bg-yellow-dark/10 text-yellow-dark' : 'text-textMuted hover:bg-bgMain'}`}>
                  <Package size={16} /> Order History
                </button>
                <button onClick={() => setActiveTab('info')}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-colors ${activeTab === 'info' ? 'bg-yellow-dark/10 text-yellow-dark' : 'text-textMuted hover:bg-bgMain'}`}>
              ...
              <div className="surface-panel p-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-bold text-textMain flex items-center gap-2">
                    <User size={20} className="text-yellow-dark" /> Account Information
                  </h2>
                  {isSupabaseConfigured && !editing && (
                    <button onClick={startEdit}
                      className="flex items-center gap-1.5 text-[12px] font-bold text-yellow-dark hover:text-yellowDeep transition-colors">
                      <Edit2 size={13} /> Edit
                    </button>
                  )}
                </div>
                ...
                {!editing && (
                  <div className="space-y-3">
                    ...
                    {!user.mobile && (
                      <p className="text-[12px] text-amber-700 bg-yellow/10 border border-yellow-dark-200 px-4 py-3 rounded-xl">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        type="text"
                        autoFocus
                        className="w-full px-4 py-3 rounded-xl border-2 border-[#F0E6C8] focus:border-yellow-darkDark outline-none text-[13px]"
                        value={editName}
                    ...
                    <div className="flex gap-3 pt-1">
                      <button onClick={handleSaveProfile} disabled={saving}
                        className="flex items-center gap-1.5 px-5 py-2.5 bg-yellow-dark hover:bg-yellow-dark text-white font-bold rounded-xl text-[13px] disabled:opacity-60 transition-colors">
                        {saving
                      ...
                      <button onClick={cancelEdit} disabled={saving}
                        className="flex items-center gap-1.5 px-5 py-2.5 border-2 border-[#F0E6C8] text-textMuted font-bold rounded-xl text-[13px] hover:bg-bgMain transition-colors">
                        <X size={14} /> Cancel
                      </button>
                    </div>
                  </div>
            ...
            {activeTab === 'orders' && (
              <div className="surface-panel p-6">
                <h2 className="text-xl font-bold text-textMain mb-6 flex items-center gap-2">
                  <Package size={20} className="text-yellow-dark" /> Order History
                  <span className="ml-auto text-sm font-normal text-textMuted">{orders.length} orders</span>
                </h2>

                {loading ? (
                  <div className="flex items-center justify-center py-16">
                    <span className="w-8 h-8 border-4 border-[#F0E6C8] border-t-yellow-dark rounded-full animate-spin" />
                  </div>
                ) : orders.length === 0 ? (
                  <div className="text-center py-16">
                    ...
                    <Link to="/products"
                      className="inline-block bg-yellow-dark hover:bg-yellow-dark text-white font-bold px-6 py-3 rounded-xl transition-colors">
                      Browse Products
                    </Link>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {orders.map((o) => {
                      ...
                      return (
                        <div key={o.id} className="border border-[#F0E6C8] rounded-xl overflow-hidden">
                          <div
                          ...
                          {isExpanded && (
                            <div className="border-t border-[#F0E6C8] bg-white p-4">
                              <div className="mb-4 pb-4 border-b border-[#F0E6C8]/50 text-sm grid grid-cols-2 gap-3">
                                <div>
                              ...
                              <div className="overflow-x-auto">
                                <table className="w-full min-w-[520px] text-sm">
                                  <thead className="text-left text-textMuted border-b border-[#F0E6C8]">
                                  <tr>
                              ...
                              <div className="mt-4 pt-4 border-t border-[#F0E6C8] text-sm space-y-1 text-right">
