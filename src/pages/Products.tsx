    ...
    <div className="space-y-10">
      <div>
        <div className="flex items-center gap-2 mb-4">
           <div className="w-1 h-4 bg-yellow-dark rounded-full"></div>
        ...
        <div className="space-y-1">
           {categories.map(cat => (
             <button
               ...
               className={`w-full text-left px-3 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center justify-between group ${activeCategory === cat ? 'bg-yellow-dark text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50 hover:text-yellow-dark'}`}
             >
              <span>{cat === 'All' ? t('cat.view_all') : t('cat.' + cat)}</span>
              <div className={`w-1.5 h-1.5 rounded-full transition-all ${activeCategory === cat ? 'bg-white' : 'bg-transparent group-hover:bg-yellow/40'}`}></div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-col gap-1.5">
          {healthConcerns.map(rem => (
            <label key={rem} className={`flex items-center gap-3 p-2 rounded-xl cursor-pointer transition-colors border ${activeRem.includes(rem) ? 'bg-yellow/10 border-yellow-dark/20' : 'border-transparent hover:bg-gray-50'}`}>
              <input
                type="checkbox"
                checked={activeRem.includes(rem)}
                onChange={() => toggle(activeRem, setActiveRem, rem)}
                className="w-4 h-4 rounded border-gray-300 text-yellow-dark focus:ring-yellow-dark"
              />
              <span className={`text-[13px] font-bold transition-colors ${activeRem.includes(rem) ? 'text-yellow-dark' : 'text-gray-500'}`}>
                {t('remedy.' + rem)}
              </span>
            </label>
          ))}
...
export default function Products() {
  ...
  const SkeletonCard = () => (
    <div className="rounded-2xl border border-[#F0E6C8]/50 bg-white overflow-hidden animate-pulse">
      <div className="aspect-square bg-[#F0E6C8]/30" />
      <div className="p-3 space-y-2">
        <div className="h-2 bg-[#F0E6C8]/40 rounded w-1/2" />
        <div className="h-3 bg-[#F0E6C8]/40 rounded w-4/5" />
        <div className="h-3 bg-[#F0E6C8]/30 rounded w-3/5" />
        <div className="flex gap-1 mt-2">
          {[1,2,3,4].map(i => <div key={i} className="h-5 bg-[#F0E6C8]/30 rounded-lg w-10" />)}
        </div>
        <div className="flex justify-between items-center pt-2 border-t border-[#F0E6C8]/30 mt-2">
          <div className="h-5 bg-[#F0E6C8]/40 rounded w-14" />
          <div className="h-8 bg-[#F0E6C8]/40 rounded-xl w-16" />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="bg-bgMain min-h-screen">
      {/* Header Panel */}
      <div className="bg-white border-b border-[#F0E6C8]/30">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:py-10 text-center">
          <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            className="text-2xl sm:text-4xl font-black font-headline text-[#1A1A1A] mb-2">{t('products.title')}</motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }}
            className="text-sm sm:text-base text-[#6B7280] font-medium">{t('products.sub')}</motion.p>
        </div>
      </div>
        ...
        <div className="flex-grow">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8">
            <div className="relative flex-grow">
               ...
               <input
                 ...
                 className="w-full h-10 sm:h-12 pl-12 pr-4 bg-white border border-gray-200 rounded-2xl focus:ring-2 focus:ring-yellow-dark focus:border-transparent outline-none shadow-sm transition-all" 
          ...
          <div className="flex items-center justify-between mb-6">
             <div className="flex items-center gap-2 text-sm font-bold text-gray-500">
                {loading ? (
                  <div className="flex items-center gap-2"><div className="w-4 h-4 border-2 border-yellow-dark border-t-transparent animate-spin rounded-full"></div> Loading...</div>
                ) : (
          ...
          {loading ? (
            ...
            <div className="bg-white border border-[#F0E6C8]/40 rounded-3xl p-10 sm:p-20 text-center flex flex-col items-center">
               <div className="w-20 h-20 bg-[#F7F6F2] rounded-full flex items-center justify-center mb-6">
                 <Search className="text-[#F0E6C8]" size={32}/>
               </div>
               <h3 className="text-xl font-black mb-2 text-[#1A1A1A]">{t('products.none')}</h3>
               <p className="text-[#6B7280] max-w-xs mx-auto mb-6 text-sm">No products found matching your filters.</p>
               <button onClick={clear} className="text-sm font-black text-[#D4A800] hover:underline">Clear all filters</button>
            </div>
          ) : (
          ...
          {/* Info Banner */}
          <div className="mt-12 p-6 sm:p-8 bg-yellow/10 border border-green-100 rounded-3xl flex flex-col sm:flex-row items-center gap-6">
             <div className="w-16 h-16 rounded-2xl bg-white shadow-sm flex items-center justify-center shrink-0">
                <CheckCircle2 size={32} className="text-yellow-dark" />
             </div>
             <div>
                <h4 className="font-black text-lg text-green-900 leading-tight">Authentic Siddha Quality</h4>
                <p className="text-sm text-yellow-dark mt-1">Every herb in our collection is sourced directly from trusted traditional practitioners and verified for purity.</p>
             </div>
          </div>
        </div>
      </div>
