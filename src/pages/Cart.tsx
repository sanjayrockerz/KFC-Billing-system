...
export default function Cart() {
  ...
  return (
    <div className="mobile-page-shell pb-28 sm:pb-8">
      <div className="bg-gradient-to-r from-[#eaf2e5] to-bgMain border-b border-[#F0E6C8]/50 py-4 sm:py-8">
        <div className="max-w-7xl mx-auto px-4 flex items-center gap-3 sm:gap-4">
          <ShoppingBag className="text-yellow-dark shrink-0" size={24} />
          <div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 sm:py-8 flex flex-col lg:flex-row gap-4 sm:gap-8">
        ...
        <div className="w-full lg:w-[62%]">
          {items.length === 0 ? (
            <div className="surface-panel p-8 sm:p-16 text-center">
              ...
              <Link to="/products" className="inline-flex items-center gap-2 bg-yellow-dark hover:bg-yellow-dark text-white font-bold px-6 py-3 rounded-xl transition-colors">
                {t('cart.browse')}
              </Link>
            </div>
          ) : (
            <div className="surface-panel overflow-hidden">
              <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-[#F0E6C8]/40">
              ...
              <div className="divide-y divide-sand/30">
                {items.map(item => (
                  <motion.div key={item.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    ...
                    <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden shrink-0 bg-gray-50 border border-[#F0E6C8]/40">
                      <img
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 flex-wrap w-full sm:w-auto">
                      <div className="flex items-center gap-0 border-2 border-[#F0E6C8] rounded-lg overflow-hidden bg-white">
          ...
          <Link to="/products" className="inline-flex items-center gap-2 text-yellow-dark font-bold text-sm mt-5 hover:gap-3 transition-all group">
            ...
            <div className="mx-auto flex max-w-7xl items-center gap-3">
              ...
              <Link to="/checkout"
                className={`flex-1 flex items-center justify-center gap-2 font-bold py-3 rounded-2xl transition-colors text-sm ${
                  items.length ? 'bg-yellow-dark hover:bg-yellow-dark text-white cursor-pointer' : 'bg-gray-100 text-gray-400 cursor-not-allowed pointer-events-none'
                }`}>
                Proceed to Checkout
              </Link>
            </div>
        ...
        <div className="w-full lg:w-[38%]">
          <div className="surface-panel p-5 sm:p-6 sticky top-24 sm:top-[110px]">
            <h2 className="font-bold text-xl font-headline text-textMain mb-5 pb-4 border-b border-[#F0E6C8]/40">{t('cart.bill_summary')}</h2>
            ...
            {/* Totals */}
            <div className="border-t border-[#F0E6C8] pt-4 space-y-2 text-sm mb-5">
