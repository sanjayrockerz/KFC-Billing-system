      ...
      <div className="mobile-page-shell py-16">
        <div className="max-w-lg mx-auto px-4">
          <div className="surface-panel p-8 text-center">
            ...
            <p className="font-bold text-yellow-dark text-base mb-1">WhatsApp Request: {booked.requestId}</p>
            ...
            <div className="bg-[#F7F6F2] rounded-xl p-4 mb-4 text-sm text-left space-y-1.5">
              ...
              {booked.discountAmount > 0 && (
                <div className="flex justify-between text-yellow-dark">
                  <span>Discount ({booked.couponCode})</span>
                  <span>−{formatCurrency(booked.discountAmount)}</span>
                </div>
              )}
              <div className="flex justify-between font-bold text-textMain text-base border-t border-[#F0E6C8] pt-2 mt-2">
                <span>Total</span><span>{formatCurrency(booked.total)}</span>
              </div>
            </div>

            <div className="bg-yellow/10 border border-green-200 rounded-2xl p-4 mb-6 text-left">
              <div className="flex items-center gap-2 mb-2">
                <MessageCircle size={18} className="text-yellow-dark" />
            ...
            <div className="space-y-3">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center justify-center gap-2 w-full bg-yellow/100 hover:bg-green-600 text-white font-bold py-3.5 rounded-xl transition-colors"
              >
                <MessageCircle size={18} /> Open WhatsApp Chat
              </a>
              {user && (
                <Link to="/profile"
                  className="flex items-center justify-center gap-2 w-full bg-yellow-dark hover:bg-yellow-dark text-white font-bold py-3.5 rounded-xl transition-colors">
                  <ShoppingBag size={18} /> View My Interactions
                </Link>
              )}
              <Link to="/products"
                className="flex items-center justify-center gap-2 w-full border-2 border-[#F0E6C8] hover:border-yellow-darkDark text-textMain font-bold py-3.5 rounded-xl transition-colors">
                Continue Browsing
              </Link>
            </div>
          </div>
  ...
  return (
    <div className="mobile-page-shell min-h-screen py-4 sm:py-10 pb-28 sm:pb-10">
      <div className="max-w-4xl mx-auto px-4">
        <button onClick={() => navigate('/cart')} className="touch-target flex items-center gap-2 mb-4 sm:mb-6 text-yellow-dark font-bold">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
                  ...
                  className="w-full px-4 py-2.5 sm:py-3 border-2 border-[#F0E6C8] focus:border-yellow-darkDark rounded-xl outline-none transition-colors" required />
              </div>
              ...
              <div>
                    ...
                    className={`flex-1 px-4 py-2.5 sm:py-3 border-2 rounded-xl outline-none transition-colors ${
                      form.phone && !isValidIndianPhone(form.phone)
                        ...
                        : form.phone && isValidIndianPhone(form.phone)
                          ...
                          : 'border-[#F0E6C8] focus:border-yellow-darkDark'
                    }`}
                    required
                  />
                </div>
                ...
                {form.phone && isValidIndianPhone(form.phone) && (
                  <p className="mt-1 text-xs text-yellow-dark font-medium">✓ Valid Indian mobile number</p>
                )}
              </div>
              ...
              <div>
                ...
                <textarea value={form.address} onChange={e => setForm({ ...form, address: e.target.value })}
                  ...
                  required
                  className="w-full px-4 py-2.5 sm:py-3 border-2 border-[#F0E6C8] focus:border-yellow-darkDark rounded-xl outline-none transition-colors resize-none" />
              </div>
              ...
              <div>
                ...
                {appliedCoupon ? (
                  <div className="flex items-center gap-2 px-4 py-2.5 bg-yellow/10 border-2 border-green-300 rounded-xl">
                    <Tag size={14} className="text-yellow-dark shrink-0" />
                    <div className="flex-1">
                      <p className="text-green-800 font-bold text-sm">{appliedCoupon.code} — {appliedCoupon.percentage}% off</p>
                      <p className="text-yellow-dark text-xs">You save {formatCurrency(appliedCoupon.discount)}</p>
                    </div>
                    <button onClick={removeCoupon} className="text-yellow-dark hover:text-red-500 transition-colors">
                      <X size={16} />
                    </button>
                  </div>
                ) : (
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-4 py-2.5 border-2 border-[#F0E6C8] focus:border-yellow-darkDark rounded-xl outline-none transition-colors text-sm font-bold tracking-wider"
                      onKeyDown={e => e.key === 'Enter' && void applyCoupon()}
                    />
                    <button
                      onClick={() => void applyCoupon()}
                      disabled={couponLoading || !couponInput.trim()}
                      className="px-4 py-2.5 bg-yellow-dark hover:bg-yellow-dark text-white font-bold rounded-xl text-sm transition-colors disabled:opacity-50 shrink-0"
                    >
                      {couponLoading ? '…' : 'Apply'}
                    </button>
                  </div>
              ...
              <div className="bg-yellow/10 border border-green-200 text-green-800 px-4 py-3 rounded-xl text-sm">
              ...
              {!user && (
                <div className="bg-yellow/10 border border-yellow-dark-200 text-amber-700 px-4 py-3 rounded-xl text-sm">
              ...
              <button onClick={handleCheckout} disabled={loading}
                className="hidden lg:flex w-full bg-yellow/100 hover:bg-green-600 text-white font-bold py-3.5 sm:py-4 rounded-xl transition-colors disabled:opacity-60 items-center justify-center gap-2 mt-2">
                {loading ? (
          ...
          <div className="surface-panel p-4 sm:p-6">
            ...
            {/* Totals breakdown */}
            <div className="mt-6 pt-5 border-t border-[#F0E6C8] space-y-2 text-sm">
              <div className="flex justify-between text-textMuted">
              ...
              <div className="flex justify-between font-bold text-textMain text-base border-t border-[#F0E6C8] pt-3 mt-2">
          ...
          <div className="lg:hidden fixed inset-x-0 bottom-0 z-20 mobile-cta-bar px-4 py-3">
            <div className="mx-auto flex max-w-4xl items-center gap-3">
              ...
              <button onClick={handleCheckout} disabled={loading}
                className="flex-1 bg-yellow/100 hover:bg-green-600 text-white font-bold py-3 rounded-2xl transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
