...
function StatCard({
  ...
  return (
    <div className="rounded-2xl border border-[#F0E6C8]/30 bg-white p-4 shadow-sm">
      ...
      <p className="break-words text-[22px] font-black leading-tight text-[#1A1A1A]">{value}</p>
    </div>
  )
}
  ...
  if (authLoading || loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F8F9FA]">
        <span className="h-10 w-10 animate-spin rounded-full border-4 border-[#F0E6C8] border-t-[#1A1A1A]" />
      </div>
    )
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#F8F9FA] p-4">
        <div className="max-w-sm rounded-3xl bg-white p-8 text-center shadow-xl">
          <h1 className="mb-2 text-2xl font-black text-[#1A1A1A]">Unauthorized</h1>
          <p className="mb-6 text-sm text-[#6B7280]">Admin access is required to view billing analytics.</p>
          <Link to="/" className="inline-flex items-center gap-2 rounded-xl bg-[#1A1A1A] px-5 py-3 text-sm font-bold text-white">
      ...
      <div className="mx-auto max-w-[1600px] px-4 py-4 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div className="admin-logo-lockup min-w-[280px]">
            <div className="admin-logo-mark shrink-0 rounded-[22px] border border-[#F0E6C8] bg-white p-2 shadow-[0_10px_28px_rgba(17,24,39,0.10)]">
              <div className="flex h-14 w-14 items-center justify-center rounded-[18px] bg-[#FCFBF7] ring-1 ring-[#F2E7D2]">
                <img src="/logo.png" alt="KFC logo" className="h-10 w-10 object-contain" />
              </div>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link
              to="/dashboard"
              className="inline-flex items-center gap-2 rounded-xl border border-[#F0E6C8]/60 bg-white px-4 py-2 text-[13px] font-bold text-[#6B7280] hover:bg-[#F7F6F2]"
            >
            ...
            <Link
              to="/pos"
              className="inline-flex items-center gap-2 rounded-xl bg-[#D4A800] px-4 py-2 text-[13px] font-bold text-white hover:bg-[#5e8c72]"
            >
        ...
        <div className="mb-4 rounded-2xl border border-[#F0E6C8]/30 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap gap-2 items-center">
            <span className="text-[11px] font-black uppercase tracking-wider text-[#6B7280] mr-1">Period:</span>
              ...
              <button
                ...
                className={`rounded-xl px-3 py-1.5 text-[12px] font-black transition-colors ${
                  analyticsDatePreset === preset
                    ...
                    : 'bg-[#F7F6F2] text-[#6B7280] hover:bg-[#F0E6C8]/40'
                }`}
              >
                {preset === 'all'
                  ? l('All Time', 'எல்லாம்')
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-3">
          <div className="rounded-2xl border border-[#F0E6C8]/30 bg-white p-5 shadow-sm xl:col-span-2">
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-[#F0E6C8]/60 bg-[#F7F6F2] px-3 py-2 text-[12px] font-bold text-[#6B7280] disabled:cursor-not-allowed disabled:opacity-50"
              >
                <Download size={13} />
                Export CSV
              </button>
            ...
            <div className="mt-4 overflow-x-auto rounded-xl border border-[#F0E6C8]/30">
              <table className="min-w-[980px] w-full text-left text-[13px]">
                ...
                <tbody className="divide-y divide-[#F0E6C8]/20">
                  {filteredBills.slice(0, 50).map((order) => {
                    ...
                    return (
                      <tr key={order.id} className="hover:bg-[#F7F6F2]/50">
                        ...
                        <td className="px-3 py-3">
                          <span className={`rounded-full px-2 py-0.5 text-[10px] font-black uppercase ${normalizeStatus(order.status) === 'completed' ? 'bg-emerald-100 text-yellow-dark' : 'bg-amber-100 text-amber-700'}`}>
                  ...
                  {filteredBills.length === 0 && (
                    <tr>
                      <td colSpan={10} className="px-4 py-10 text-center text-[13px] text-[#6B7280]">
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-[#F0E6C8]/30 bg-white p-5 shadow-sm">
              ...
              {analytics.topProducts.length > 0 ? (
                  ...
                  {analytics.topProducts.slice(0, 8).map((product, index) => (
                    <div key={`${product.name}-${product.variant}-${index}`} className="rounded-xl bg-[#F7F6F2] p-3">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="font-black text-[#1A1A1A]">{product.name}</p>
                          <p className="text-[11px] text-[#7A846F]">{product.variant || 'Variant not set'}</p>
                        </div>
                        <p className="text-right text-[12px] font-black text-yellow-dark">{formatCurrency(product.revenue)}</p>
                      </div>
                      <div className="mt-2 flex items-center justify-between text-[11px] font-bold text-[#6B7280]">
                ...
                <p className="py-8 text-center text-[13px] text-[#6B7280]">No product sales in selected period.</p>
              )}
            </div>
                ...
                <div className="space-y-3">
                  {analytics.topCategories.map((category, index) => (
                    <div key={`${category.name}-${index}`} className="rounded-xl bg-[#F7F6F2] p-3">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="font-black text-[#1A1A1A]">{category.name}</p>
                          <p className="text-[11px] text-[#7A846F]">Qty sold: {Math.round(category.qty)}</p>
                        </div>
                        <p className="text-[12px] font-black text-yellow-dark">{formatCurrency(category.revenue)}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="py-8 text-center text-[13px] text-[#6B7280]">No category data in selected period.</p>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-[#F0E6C8]/30 bg-white p-5 shadow-sm">
              ...
              {analytics.topCoupons.length > 0 ? (
                <div className="space-y-3">
                  {analytics.topCoupons.map((coupon) => (
                    <div key={coupon.code} className="flex items-center justify-between gap-3 rounded-xl bg-[#F7F6F2] p-3">
                      <div>
                        <p className="font-black text-[#1A1A1A]">{coupon.code}</p>
                        <p className="text-[11px] text-[#7A846F]">Used {coupon.usage} time(s)</p>
                      </div>
                      <div className="text-right">
                        <p className="font-black text-[#1A1A1A]">{formatCurrency(coupon.discounts)}</p>
                ...
                <p className="py-8 text-center text-[13px] text-[#6B7280]">No coupon usage yet.</p>
              )}
            </div>
          </div>
        </div>
