          ...
          >
            ...
            <div className="flex items-start gap-3 px-4 pt-2 pb-3 border-b border-gray-100 shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-[14px] font-black text-[#1A1A1A] leading-tight line-clamp-1">
                  {product.name}
                </h3>
                {selected && (
                  <div className="mt-0.5 flex items-baseline gap-1.5">
                    <span className="text-[15px] font-black text-[#1A1A1A] tabular-nums">
                      {formatCurrency(selected.price)}
                    </span>
                    {selected.sizeLabel && (
                      <span className="text-[11px] text-[#6B7280]">{selected.sizeLabel}</span>
                    )}
                  </div>
                )}
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain px-4 py-3">
              <p className="mb-2.5 text-[10px] font-black uppercase tracking-wider text-[#6B7280]">
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        isSelected
                          ? 'bg-[#1A1A1A]/6 ring-2 ring-[#1A1A1A]'
                          : outOfStock
                          ? 'cursor-not-allowed opacity-45 ring-1 ring-gray-200'
                          : 'ring-1 ring-gray-200 hover:ring-[#D4A800] active:bg-gray-50',
                      ].join(' ')}
                    >
                      {/* Radio circle */}
                      <span
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {v.sizeLabel && v.sizeLabel !== v.variantName && (
                          <p className="text-[11px] text-[#6B7280] mt-0.5">{v.sizeLabel}</p>
                        )}
                        {outOfStock && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center overflow-hidden rounded-xl border border-[#D5DAD0] bg-[#F7F6F2]">
                  <button
                    ...
                    disabled={qty <= 1}
                    className="flex h-11 w-11 items-center justify-center text-[#1A1A1A] transition-colors hover:bg-[#E8EDE4] disabled:opacity-40"
                    aria-label="Decrease"
                  >
                    <Minus size={14} />
                  </button>
                  <span className="w-10 text-center text-[15px] font-black text-[#1A1A1A] tabular-nums">
                    {qty}
                  </span>
                  <button
                    type="button"
                ...
                <motion.button
                  ...
                  className={[
                    ...
                    added
                      ...
                      : 'bg-[#1A1A1A] text-white hover:bg-[#1e2817]',
                  ].join(' ')}
                >
                  {added ? (
                    <>
