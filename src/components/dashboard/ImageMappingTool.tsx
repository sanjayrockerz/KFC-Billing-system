  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-black text-[#1A1A1A]">Image Mapping</h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-white border border-[#F0E6C8]/50 rounded-xl text-[12px] font-bold text-[#6B7280] hover:bg-[#F7F6F2] transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-white border border-[#F0E6C8]/50 rounded-xl text-[12px] font-bold text-[#6B7280] hover:bg-[#F7F6F2] transition-colors disabled:opacity-40"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-[#1A1A1A] text-white rounded-xl text-[12px] font-black hover:bg-[#1e2817] transition-colors disabled:opacity-40"
          >
            <Upload size={13} />
      ...
      {notice && (
        <div className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-[12px] font-bold ${
          notice.type === 'ok' ? 'bg-yellow/10 text-yellow-dark border border-emerald-200' : 'bg-red-50 text-red-700 border border-red-200'
        }`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          { label: 'Total Images', value: stats.totalImages, color: 'text-[#1A1A1A]' },
          { label: 'Total Products', value: stats.totalProducts, color: 'text-[#1A1A1A]' },
          { label: 'Mapped', value: stats.mapped, color: 'text-yellow-dark' },
          { label: 'Unmapped Images', value: stats.unmappedImages, color: stats.unmappedImages > 0 ? 'text-yellow-dark' : 'text-yellow-dark' },
          { label: 'Duplicates', value: stats.duplicates, color: stats.duplicates > 0 ? 'text-red-600' : 'text-yellow-dark' },
          { label: 'Products Missing', value: stats.productsMissingImages, color: stats.productsMissingImages > 0 ? 'text-yellow-dark' : 'text-yellow-dark' },
        ].map(s => (
          <div key={s.label} className="bg-white rounded-xl border border-[#F0E6C8]/30 px-3 py-3 shadow-sm">
            <p className="text-[10px] font-bold uppercase tracking-wider text-[#6B7280]">{s.label}</p>
        ...
        {(['map', 'review'] as const).map(v => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-[12px] font-black transition-colors ${
              view === v ? 'bg-white text-[#1A1A1A] shadow-sm' : 'text-[#6B7280]'
            }`}
          >
      ...
      {view === 'map' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 min-h-[600px]">
          {/* LEFT: Image gallery */}
          <div className="bg-white rounded-2xl border border-[#F0E6C8]/30 shadow-sm flex flex-col">
            <div className="flex items-center justify-between gap-2 p-4 border-b border-[#F0E6C8]/20">
              <h3 className="text-[13px] font-black text-[#1A1A1A]">
                Images_V2 <span className="text-[#6B7280] font-bold">({filteredImages.length})</span>
              </h3>
                ...
                {(['all', 'unmapped', 'mapped'] as const).map(f => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-md text-[10px] font-black transition-colors capitalize ${
                      imageFilter === f ? 'bg-white text-[#1A1A1A] shadow-sm' : 'text-[#6B7280]'
                    }`}
                  >
                    {f}
                  </button>
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        isSelected
                          ? 'border-[#1A1A1A] shadow-md ring-2 ring-[#1A1A1A]/20'
                          : isDup
                          ? 'border-red-400'
                          : isMapped
                          ? 'border-emerald-400'
                          : 'border-[#F0E6C8]/40 hover:border-[#D4A800]',
                      ].join(' ')}
                    >
                      {/* Image */}
                      ...
                      {/* Status badge */}
                      <div className="p-1.5">
                        <p className="text-[9px] font-bold text-[#6B7280] leading-tight truncate" title={filename}>
                          {filename.replace('WhatsApp Image ', '').replace(' PM', '').replace(' AM', '')}
                        </p>
                        {isMapped && mappedTo && (
                          <p className="text-[8px] font-black text-yellow-dark truncate mt-0.5">
                      ...
                      {/* Corner check */}
                      {isMapped && !isDup && (
                        <div className="absolute right-1 top-1 h-5 w-5 rounded-full bg-yellow/100 flex items-center justify-center">
          ...
          <div className="bg-white rounded-2xl border border-[#F0E6C8]/30 shadow-sm flex flex-col">
            {selectedImage ? (
              <>
                ...
                <div className="p-4 border-b border-[#F0E6C8]/20">
                  <div className="flex items-start gap-3">
                    <div className="h-20 w-20 shrink-0 rounded-xl overflow-hidden bg-[#F0F2EE] border border-[#F0E6C8]/40">
                      <img
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-black text-[#1A1A1A] leading-tight break-all">
                        {selectedImage}
                      </p>
                      {mappedFiles.has(selectedImage) ? (
                        <div className="mt-1.5 flex items-center gap-1.5">
                          <span className="text-[10px] font-black text-yellow-dark bg-yellow/10 px-2 py-0.5 rounded-full">
                        ...
                        <p className="mt-1 text-[10px] font-bold text-yellow-dark bg-yellow/10 px-2 py-0.5 rounded-full w-fit">
                          Unmapped
                        </p>
                      )}
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-8 pr-3 py-2.5 bg-[#F7F6F2] rounded-xl text-[13px] font-bold border border-[#F0E6C8]/40 outline-none focus:border-[#D4A800] transition-colors"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full appearance-none px-3 py-2.5 bg-[#F7F6F2] rounded-xl text-[13px] font-bold border border-[#F0E6C8]/40 outline-none focus:border-[#D4A800] transition-colors pr-8"
                      >
                  ...
                  {selectedProductId && (() => {
                    ...
                    if (!prod) return null
                    return (
                      <div className="rounded-xl bg-[#F7F8F5] border border-[#F0E6C8]/40 p-3 flex items-center gap-3">
                        <Tag size={15} className="text-[#D4A800] shrink-0" />
                        <div className="min-w-0">
                          <p className="text-[13px] font-black text-[#1A1A1A] truncate">{prod.name}</p>
                          <p className="text-[11px] text-[#6B7280]">{prod.category} · ₹{prod.price}</p>
                          {prod.hasVariants && (
                            <p className="text-[10px] font-bold text-[#D4A800]">
                ...
                {/* Map button */}
                <div className="p-4 border-t border-[#F0E6C8]/20 flex gap-2">
                  <button
                    type="button"
                    disabled={!selectedProductId}
                    onClick={handleSaveMapping}
                    className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-[#1A1A1A] text-white text-[13px] font-black hover:bg-[#1e2817] disabled:opacity-40 transition-colors"
                  >
              ...
              <div className="flex flex-col items-center justify-center h-full py-16 text-center px-8">
                ...
                <h3 className="text-[14px] font-black text-[#1A1A1A] mb-1">Select an image</h3>
      ...
      {view === 'review' && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          {/* Mapped */}
          <div className="bg-white rounded-2xl border border-[#F0E6C8]/30 shadow-sm">
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#F0E6C8]/20">
              <h3 className="text-[13px] font-black text-[#1A1A1A]">
                ✓ Mapped ({Object.keys(mappings).length})
              </h3>
            </div>
            <div className="divide-y divide-[#F0E6C8]/20 max-h-[400px] overflow-y-auto">
              {Object.entries(mappings).length === 0 ? (
                ...
                Object.entries(mappings).map(([key, filename]) => (
                  <div key={key} className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] font-black text-[#1A1A1A] truncate">{labelForKey(key)}</p>
                      <p className="text-[10px] text-[#6B7280] truncate">{filename}</p>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-[#F0E6C8]/30 shadow-sm">
              ...
              <div className="max-h-[200px] overflow-y-auto divide-y divide-[#F0E6C8]/20">
                {images.filter(f => !mappedFiles.has(f)).length === 0 ? (
                  <p className="p-4 text-center text-[12px] font-bold text-yellow-dark">All images mapped ✓</p>
                ) : (
                  images.filter(f => !mappedFiles.has(f)).map(f => (
                    <div key={f} className="flex items-center gap-3 px-4 py-2">
                      ...
                      <p className="text-[11px] text-[#6B7280] truncate flex-1">{f}</p>
                      <button
                        type="button"
                        onClick={() => { setSelectedImage(f); setView('map') }}
                        className="text-[10px] font-bold text-[#D4A800] hover:underline shrink-0"
                      >
                        Map →
                      </button>
                    </div>
            ...
            <div className="bg-white rounded-2xl border border-[#F0E6C8]/30 shadow-sm">
              ...
              <div className="max-h-[200px] overflow-y-auto divide-y divide-[#F0E6C8]/20">
                {unmappedProducts.length === 0 ? (
                  <p className="p-4 text-center text-[12px] font-bold text-yellow-dark">All products have images ✓</p>
                ) : (
                  unmappedProducts.slice(0, 40).map(p => (
                    <div key={p.id} className="flex items-center justify-between px-4 py-2">
                      <div className="min-w-0">
                        <p className="text-[12px] font-bold text-[#1A1A1A] truncate">{p.name}</p>
                        <p className="text-[10px] text-[#6B7280]">{p.category}</p>
                      </div>
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="text-[10px] font-bold text-[#D4A800] hover:underline ml-3 shrink-0"
                      >
                        Map →
                      </button>
                    </div>
      ...
      {/* ── UPLOAD PROGRESS ── */}
      {uploading && (
        <div className="bg-white rounded-2xl border border-[#F0E6C8]/30 p-5 shadow-sm space-y-3">
          <div className="flex items-center gap-2">
            <RefreshCw size={15} className="text-[#D4A800] animate-spin" />
            <span className="text-[13px] font-black text-[#1A1A1A]">Uploading images…</span>
          </div>
          <div className="h-2 bg-[#F0F2EE] rounded-full overflow-hidden">
            <div
              className="h-full bg-[#D4A800] rounded-full transition-all duration-300"
      ...
      {uploadResults.length > 0 && !uploading && (
        <div className="bg-white rounded-2xl border border-[#F0E6C8]/30 shadow-sm">
          <div className="px-4 py-3 border-b border-[#F0E6C8]/20 flex items-center justify-between">
            <h3 className="text-[13px] font-black text-[#1A1A1A]">Upload Results</h3>
            <div className="flex gap-3 text-[11px] font-black">
              <span className="text-yellow-dark">✓ {uploadResults.filter(r => r.status === 'ok').length} success</span>
              {uploadResults.some(r => r.status === 'error') && (
                <span className="text-red-500">✗ {uploadResults.filter(r => r.status === 'error').length} failed</span>
              )}
            </div>
          </div>
          <div className="max-h-[300px] overflow-y-auto divide-y divide-[#F0E6C8]/20">
            {uploadResults.map((r, i) => (
              <div key={i} className="flex items-start gap-3 px-4 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[12px] font-bold text-[#1A1A1A] truncate">{r.productName}</p>
                  <p className="text-[10px] text-[#6B7280] truncate">{r.file}</p>
                  {r.status === 'error' && (
                    <p className="text-[10px] text-red-500 mt-0.5">{r.message}</p>
                  )}
                  {r.url && (
                    <p className="text-[10px] text-yellow-dark truncate">{r.url}</p>
                  )}
                </div>
              </div>
            ))}
      ...
      {confirmUpload && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 space-y-4">
            <div className="flex items-start gap-3">
              <div className="h-10 w-10 rounded-xl bg-yellow/10 flex items-center justify-center shrink-0">
                <AlertTriangle size={20} className="text-yellow-dark" />
              </div>
              <div>
                <h3 className="text-[15px] font-black text-[#1A1A1A]">Confirm Upload & Apply</h3>
            ...
            <div className="bg-[#F7F8F5] rounded-xl p-3 space-y-1 text-[11px] font-bold text-[#6B7280] max-h-[200px] overflow-y-auto">
              {Object.entries(mappings).map(([key, file]) => (
                <div key={key} className="flex items-center gap-2">
                  <span className="text-[#D4A800]">→</span>
                  <span className="text-[#1A1A1A] truncate">{labelForKey(key)}</span>
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl border border-[#F0E6C8]/60 text-[13px] font-black text-[#6B7280] hover:bg-[#F7F6F2] transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-[#1A1A1A] text-white text-[13px] font-black hover:bg-[#1e2817] transition-colors"
              >
                Upload & Apply
              </button>
            </div>
