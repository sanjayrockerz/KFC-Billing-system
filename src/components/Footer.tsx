...
export default function Footer() {
  ...
  return (
    <footer className="bg-[#1A1A1A] text-gray-300 mt-12 sm:mt-16">
      <div className="max-w-7xl mx-auto px-4 py-10 sm:py-14 grid grid-cols-1 md:grid-cols-4 gap-8 sm:gap-10">
        <div className="md:col-span-1">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-9 h-9 bg-yellow-dark rounded-xl flex items-center justify-center"><Leaf size={18} className="text-white" /></div>
            <div>
              <p className="font-bold text-white text-base sm:text-lg font-headline leading-tight">{BRAND_EN}</p>
              <p className="text-[10px] uppercase tracking-[0.2em] text-yellow font-bold">{BRAND_SUBTITLE}</p>
            </div>
          </div>
          <p className="text-sm text-gray-400 leading-relaxed mb-4">{t('footer.desc')}</p>
          <p className="text-xs text-yellow font-bold uppercase tracking-wider">{t('footer.tags')}</p>
        </div>
        <div>
          <h5 className="text-white font-bold mb-4 sm:mb-5 uppercase text-xs tracking-widest">{t('footer.shop')}</h5>
          <ul className="flex flex-col gap-2.5 text-sm">
            {categories.map(c => (
              <li key={c}><Link to={`/products?cat=${encodeURIComponent(c)}`} className="hover:text-yellow transition-colors">{t('cat.' + c)}</Link></li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul className="flex flex-col gap-2.5 text-sm">
            {remedies.map(r => (
              <li key={r}><Link to={`/products?remedy=${encodeURIComponent(r)}`} className="hover:text-yellow transition-colors">{t('remedy.' + r)}</Link></li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul className="flex flex-col gap-4 text-sm">
            <li className="flex items-start gap-3">
              <MapPin size={15} className="text-yellow mt-0.5 shrink-0" />
            ...
            {contactNumbers.map((contact) => (
              <li key={contact.label} className="flex items-start gap-3">
                <Phone size={15} className="text-yellow mt-0.5 shrink-0" />
            ...
            <li className="flex items-start gap-3 flex-wrap">
              <Mail size={15} className="text-yellow mt-0.5 shrink-0" />
