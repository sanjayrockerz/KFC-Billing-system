          ...
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 290, damping: 28 }} className="fixed right-0 top-0 h-full w-full max-w-sm bg-white z-50 flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div className="flex items-center gap-2">
                <ShoppingBag className="text-yellow-dark" size={20} />
            ...
            <div className="flex-grow overflow-y-auto px-4 py-3 space-y-3">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-4 text-center py-16">
                  ...
                  <Link to="/products" onClick={onClose} className="text-yellow-dark font-bold text-sm hover:underline">{t('drawer.browse')}</Link>
                </div>
              ) : (
                <AnimatePresence>
                  {items.map(item => (
                    <motion.div key={item.id} layout initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex gap-3 p-3 bg-bgMain rounded-xl border border-[#F0E6C8]/40">
                      ...
                      <div className="flex-grow min-w-0">
                        ...
                        <p className="text-[11px] text-yellow-dark font-bold mb-2">{item.unitLabel} • {formatCurrency(item.basePrice)}</p>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-0.5 border border-[#F0E6C8] rounded-lg bg-white overflow-hidden">
            ...
            {items.length > 0 && (
              <div className="px-4 py-4 border-t border-gray-100 space-y-3">
                ...
                <Link to="/cart" onClick={onClose} className="flex items-center justify-center w-full bg-yellow-dark hover:bg-yellow-dark text-white font-bold py-3 rounded-xl transition-colors">
                  {t('drawer.view_cart')}
                </Link>
              </div>
            )}
          ...
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 290, damping: 28 }} className="fixed right-0 top-0 h-full w-full max-w-sm bg-white z-50 flex flex-col shadow-2xl">
            ...
            <div className="flex-grow overflow-y-auto px-4 py-3 space-y-3">
              {items.length === 0 ? (
                ...
                <AnimatePresence>
                  {items.map(item => (
                    <motion.div key={item.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -20 }} className="flex gap-3 p-3 bg-[#FFF8E7] rounded-xl border border-[#F0E6C8]/50">
                      ...
                      <div className="flex-grow min-w-0">
                        ...
                        <p className="text-xs text-yellow-dark font-bold">{t('cat.' + item.category)}</p>
                        <div className="flex items-center justify-between mt-2">
                          <span className="font-bold text-textMain">{item.unitLabel} • {formatCurrency(item.offerPrice || item.price)}</span>
                          <button onClick={() => { add(item); onClose() }} className="flex items-center gap-1 bg-yellow-dark hover:bg-yellow-dark text-white text-xs font-bold px-2.5 py-1.5 rounded-lg transition-colors">
                            <ShoppingBag size={11} /> {t('drawer.add')}
                          </button>
                        </div>
                      </div>
                      <button onClick={() => toggle(item)} className="self-start p-1 text-yellow-dark hover:text-yellow-dark"><X size={14} /></button>
                    </motion.div>
                  ))}
                </AnimatePresence>
              )}
