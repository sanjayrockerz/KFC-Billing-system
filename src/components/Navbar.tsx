...
export default function Navbar() {
  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-0.5 bg-[#1A1A1A]/95 px-3 py-2 text-center text-[11px] sm:text-xs font-medium tracking-wide text-yellow shadow-soft">
        ...
        <button onClick={() => setLang(lang === 'en' ? 'ta' : 'en')} className="hidden sm:inline-flex items-center rounded-full bg-white/8 p-0.5 text-[10px] font-bold text-white transition-colors shadow-soft">
          <span className={`px-2 py-0.5 rounded-full transition-colors ${lang === 'en' ? 'bg-white text-[#1A1A1A]' : 'text-white/70'}`}>EN</span>
          <span className={`px-2 py-0.5 rounded-full transition-colors ${lang === 'ta' ? 'bg-white text-[#1A1A1A]' : 'text-white/70'}`}>தமிழ்</span>
        </button>
      </div>

      <header className="sticky top-0 z-40 glass border-b border-[#F0E6C8]/40 shadow-sm">
        <nav className="mx-auto flex max-w-7xl items-center justify-between gap-2 sm:gap-3 px-3 py-2.5 sm:px-4 sm:py-3 lg:gap-4">
          <Link to="/" className="group flex min-w-0 items-center gap-2 sm:gap-2.5">
            <div className="w-10 h-10 sm:w-11 sm:h-11 bg-white rounded-full flex items-center justify-center overflow-hidden shadow-sm shrink-0 border border-[#F0E6C8]/40 group-hover:opacity-90 transition-opacity">
            ...
            <div className="flex min-w-0 flex-col leading-none">
              ...
              <p className="hidden text-[9px] font-bold uppercase tracking-[0.15em] text-yellow-dark md:block">{BRAND_SUBTITLE}</p>
            </div>
          </Link>
          ...
          <form onSubmit={handleSearch} className="hidden flex-grow max-w-xl md:flex min-w-0">
            <div className="relative w-full flex rounded-full bg-white/90 shadow-soft transition-shadow focus-within:shadow-hover">
              ...
              <button type="submit" className="h-10 sm:h-11 px-5 bg-gradient-to-r from-yellow-dark to-yellow-dark text-white text-sm font-bold rounded-full transition-all mr-0.5 my-0.5 shadow-md hover:scale-[1.02]">
                {t('nav.search')}
              </button>
            </div>
          </form>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <button
              ...
              className="flex items-center rounded-full bg-[#F7F6F2] border border-[#F0E6C8]/60 px-1 py-0.5 text-[10px] font-black text-[#1A1A1A] shrink-0 mr-0.5"
            >
              <span className={`px-1.5 py-0.5 rounded-full transition-colors ${lang === 'en' ? 'bg-[#1A1A1A] text-white' : 'text-[#6B7280]'}`}>EN</span>
              <span className={`px-1.5 py-0.5 rounded-full transition-colors ${lang === 'ta' ? 'bg-[#1A1A1A] text-white' : 'text-[#6B7280]'}`}>த</span>
            </button>
            <motion.button whileTap={{ scale: 0.88 }} onClick={() => setShowFav(true)} className="relative rounded-full hover:bg-yellow/20 transition-colors touch-target">
              <Heart size={18} className="text-textMuted sm:size-[20px]" />
              {favCount > 0 && <span className="absolute -top-0.5 -right-0.5 bg-yellow-dark text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{favCount}</span>}
            </motion.button>
            <motion.button whileTap={{ scale: 0.88 }} onClick={() => setShowCart(true)} className="relative rounded-full hover:bg-yellow/20 transition-colors touch-target">
              <ShoppingCart size={18} className="text-textMuted sm:size-[20px]" />
              {count > 0 && <span className="absolute -top-0.5 -right-0.5 bg-yellow-dark text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{count}</span>}
            </motion.button>
            <button onClick={() => setMobileOpen(!mobileOpen)} className="lg:hidden rounded-full hover:bg-yellow/20 transition-colors ml-1 touch-target">
        ...
        <AnimatePresence>
          {mobileOpen && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
              className="lg:hidden overflow-hidden border-t border-[#F0E6C8]/50 bg-white px-4 py-4 flex flex-col gap-3">
              <form onSubmit={handleSearch} className="flex">
                <input value={query} onChange={e => setQuery(e.target.value)} type="text" placeholder={t('nav.search_placeholder')}
                  className="flex-grow h-10 px-3 rounded-l-lg border-2 border-[#F0E6C8] focus:border-yellow-dark outline-none text-sm" />
                <button type="submit" className="h-10 px-4 bg-yellow-dark text-white text-sm font-bold rounded-r-lg">{t('nav.search')}</button>
              </form>
              ...
              {user && (
                <button
                  ...
                  className="w-full py-2.5 px-3 bg-white border border-[#F0E6C8] rounded-lg text-sm font-semibold text-red-500"
                >
                  {t('nav.logout')}
                </button>
              )}
