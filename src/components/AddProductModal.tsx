      ...
      if (categoryName) {
        ...
        if (!existingCategory) {
          const { data: insertedCategory, error: categoryInsertError } = await supabase
            .from('categories')
            .insert({ name_en: categoryName, name_ta: '', is_active: true, sort_order: categoryOptions.length + 1 })
            .select('id')
            .single()
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          <div>
            ...
            <input 
              ...
              placeholder="E.g. Korean Chicken Combo"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
              ...
              {categoryMode === 'select' ? (
                <div className="flex gap-1">
                  <select
                    ...
                    className="flex-1 w-full px-4 py-3 bg-[#F7F6F2] border border-[#EAD7B7]/60 rounded-xl focus:outline-none focus:border-[#D4A800] text-[13px] font-bold appearance-none"
                  >
                  ...
                  <button
                    ...
                    className="px-2 py-3 text-[10px] font-black text-[#D4A800] bg-[#F7F6F2] border border-[#EAD7B7]/60 rounded-xl hover:bg-[#EAD7B7]/40 transition-colors shrink-0"
                ...
                <div className="flex gap-1">
                  <input
                    ...
                    value={formData.category}
                    onChange={e => setFormData({...formData, category: e.target.value})}
                    className="flex-1 w-full px-4 py-3 bg-[#F7F6F2] border border-[#EAD7B7]/60 rounded-xl focus:outline-none focus:border-[#D4A800] text-[13px] font-bold"
            ...
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-3 bg-[#F7F6F2] border border-[#EAD7B7]/60 rounded-xl focus:outline-none focus:border-[#D4A800] text-[13px] font-bold text-right"
                placeholder="0"
              />
            </div>
          </div>
