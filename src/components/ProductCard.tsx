    ...
    >
      ...
      <motion.button
        ...
        className={`absolute right-1.5 top-1.5 z-10 hidden xl:flex h-7 w-7 items-center justify-center rounded-full border transition-colors ${
          fav ? 'border-rose-200 bg-rose-50' : 'border-[#F0E6C8] bg-white/90'
        }`}
        type="button"
        aria-label={fav ? 'Remove from favourites' : 'Add to favourites'}
      >
        <Heart size={13} className={fav ? 'fill-rose-500 text-yellow-dark' : 'text-slate-400'} />
      </motion.button>

      {discount > 0 && (
        <div className="absolute left-2 top-2 z-10 rounded-full bg-yellow/100 px-1.5 py-0.5 text-[10px] font-black text-white">
          {discount}% OFF
        </div>
      )}
      ...
      <div className="flex flex-1 flex-col gap-2 p-2.5 sm:p-3">
        ...
        <div className="flex flex-1 flex-col text-left">
          ...
          >
            <h3 className="line-clamp-2 min-h-[2.4rem] text-[12px] font-semibold leading-[1.4] text-[#1A1A1A] ta-text hover:text-[#1e2817]">
              {displayName}
            </h3>
          </button>
          ...
          <div className="mt-1 flex items-baseline gap-1">
            <span className="text-[13px] font-black text-[#1A1A1A] tabular-nums">
              ...
              {product.hasVariants ? (
                variantInCart ? (
                  ...
                  <motion.button
                    ...
                    type="button"
                    onClick={openModal}
                    className="flex w-full items-center justify-between rounded-xl bg-[#1A1A1A]/8 px-3 py-2 text-[11px] font-black text-[#1A1A1A] ring-1 ring-[#1A1A1A]/20 active:bg-[#1A1A1A]/12"
                  >
                  ...
                  <motion.button
                    ...
                    type="button"
                    onClick={handleAdd}
                    className="flex w-full items-center justify-center gap-1 rounded-xl border-2 border-[#1A1A1A] px-3 py-2 text-[12px] font-black text-[#1A1A1A] transition-colors active:bg-[#1A1A1A] active:text-white"
                  >
                    {l('ADD', 'சேர்')}
                ...
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 4 }}
                  transition={{ duration: 0.14 }}
                  className="flex items-center justify-between rounded-xl bg-[#1A1A1A] px-1.5 py-1"
                >
                  <button
                    type="button"
                ...
                <motion.button
                  ...
                  type="button"
                  onClick={handleAdd}
                  className="flex w-full items-center justify-center rounded-xl border-2 border-[#1A1A1A] px-3 py-2 text-[12px] font-black text-[#1A1A1A] transition-colors active:bg-[#1A1A1A] active:text-white"
                >
                  {l('ADD', 'சேர்')}
                </motion.button>
              )}
