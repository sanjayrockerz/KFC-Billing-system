...
/**
 * Korean Fried Chicken - core state management
...
export interface Product {
  ...
  // New Korean Fried Chicken POS fields
  sku?: string
  barcode?: string
  brand?: string
...
export interface CartItem extends Product {
  ...
  // Korean Fried Chicken POS billing fields
  cartItemId: string
...
const mapDbProduct = (input: unknown, categoriesById: Record<string, string> = {}): Product => {
  ...
  return {
    ...
    // Korean Fried Chicken POS mapping
...
const withRequiredKfcProducts = (products: Product[]): Product[] => {
  const seen = new Set(products.filter(product => product.isActive).map(product => product.name.trim().toLowerCase()))
  const missing = (PRODUCTS as Product[]).filter(product => !seen.has(product.name.trim().toLowerCase()))
...
export const useProductStore = create<ProductState>((set, get) => ({
  ...
  fetchProducts: async (force = false) => {
    ...
    if (!isSupabaseConfigured) {
      set({
        products: withRequiredKfcProducts([]),
        loading: false,
        error: null,
        lastFetch: Date.now(),
      })
    ...
    try {
      ...
      set({ products: withRequiredKfcProducts(normalized), loading: false, lastFetch: Date.now() })
    } catch (err) {
      set({
        products: withRequiredKfcProducts([]),
        error: null,
...
export const useAdminAuthStore = create<AdminAuthState>()(
  persist(
    ...
    { 
      name: 'korean-fried-chicken-admin-session',
